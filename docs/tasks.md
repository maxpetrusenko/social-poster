...
Last updated: 2026-06-14 (liked-post publish obligation and repair policy)
...
X liked-post content-machine note 2026-05-24, updated 2026-06-14: a liked post creates a publishing obligation. Eligible liked posts must queue unless they are duplicate, own/reply/empty, or hit hard brand-safety blocks. Writer quality rejection or writer unavailability now repairs the post with a faithful deterministic fallback instead of skipping queue creation. The fallback preserves the source's concrete point, avoids copying source-owned first person into Max's voice, and keeps recovered primary study URLs when a liked post references research. `pipeline_runs.steps` is the evidence trail for future gbrain ingestion: source capture, gbrain-context placeholder, source verification, AI draft/judge or fallback-repaired, packet readiness, queue slot, and learning readiness. Matrix/Telegram failure alerts remain for operational publish failures, but writer repair is not a dropped post.

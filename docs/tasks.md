...
X liked-post posting quality note 2026-05-23: Max is happy with the X liked-post path after commit `e448f8d` and the FreeLLMAPI repo-bookmark publish. Posting is paused unless Max explicitly asks to publish. Long-form X posts are allowed on Max's account, so liked-post autoposts should pass full text through Bird as one post and use `1/2` numbering only for intentional multi-post threads. Source-owned launches stay source-owned: if OpenAI/Anthropic/Cursor/etc. says "we launched", Max's post attributes the launch to that source and only makes first-hand experience claims when Hermes/research/browser evidence exists. Liked AI-topic posts should be eligible by default; keep high-controversy topics filtered. Linear follow-up: `MAX-158`.

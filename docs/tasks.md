...
Last updated: 2026-05-24 (calendar app-timezone regression + deploy reliability)
...
X liked-post posting quality note 2026-05-23: Max is happy with the X liked-post path after commit `e448f8d` and the FreeLLMAPI repo-bookmark publish. In the liked-post worker, Max liking a post is the publish signal once the worker is enabled; do not treat "eligible" as a holding state. Skip only duplicates, own posts, replies, empty text, and hard brand-safety blocks. Long-form X posts are allowed on Max's account, so liked-post autoposts should pass full text through Bird as one post and use `1/2` numbering only for intentional multi-post threads. LinkedIn image publishing works when the post has media; when a liked X post has no attached media but does include an external/GitHub link, the liked-post flow should resolve that link's Open Graph image and snapshot it before publishing. Video/demo likes include the original X URL in the X main post so the source video embeds, then reply with source attribution; LinkedIn uploads the video natively and puts source attribution in the first comment. Opinion posts can keep the main text clean while provenance lives in reply/comment. Benchmark/video posts should preserve concrete numeric evidence from the original when the numbers make the post useful for bookmarks. Source-owned launches stay source-owned: if OpenAI/Anthropic/Cursor/etc. says "we launched", Max's post attributes the launch to that source and only makes first-hand experience claims when Hermes/research/browser evidence exists. Remove obvious AI-social phrasing from generated takes: antithesis frames, fake triads, generic framework voice, and empty words like game-changing/cutting-edge/revolutionary/unlock/redefine. Capability decisions are now sketched in `docs/plans/capability-decision-flow.md`, with V0 schema in `platform_capabilities`. Linear follow-up: `MAX-158`.

X liked-post content-machine note 2026-05-24: a liked post now creates a publishing obligation, not a quality-gated candidate. Writer quality failures move through `x-like:research` and `x-like:faithful-curation` run steps, then publish bounded close-to-original curation instead of skipping. Operational failures are the only Telegram-notifiable path: writer/API unavailable, auth/token failures, rate limits, provider/network failures, storage/queue failures, and repeated platform publish failures. Telegram alerts include the run id, workspace, source URL, retry count, dashboard pipeline URL, platform, classification, and exact error. `pipeline_runs.steps` is the first-slice evidence trail for future gbrain ingestion: source capture, gbrain-context placeholder, source verification, draft/judge or fallback, packet readiness, publish outcome, Telegram result, and learning readiness.

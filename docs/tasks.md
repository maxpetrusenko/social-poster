...
Last updated: 2026-06-10 (liked-post hourly queue and failure alerts)
...
X liked-post content-machine note 2026-05-24, updated 2026-06-10: a liked post creates a publishing obligation, but weak or failed writing must not silently publish fallback copy. Writer unavailable and writer quality rejected now fail the run, add `x-like:review-needed`, skip queue creation, and notify configured failure channels. Matrix is supported through `MATRIX_HOMESERVER_URL`, `MATRIX_ACCESS_TOKEN`, and `SOCIAL_POSTER_MATRIX_ROOM_ID` with `MATRIX_ROOM_ID` as fallback; Telegram remains optional. Alerts include run id, workspace, source URL, retry count, dashboard pipeline URL, platform, classification, and exact error. `pipeline_runs.steps` is the evidence trail for future gbrain ingestion: source capture, gbrain-context placeholder, source verification, AI draft/judge, review-needed or packet readiness, queue slot, and learning readiness.
